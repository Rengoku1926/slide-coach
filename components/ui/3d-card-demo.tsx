...
//
// 1) We define a context whose value is always a tuple [boolean, Dispatch<...>].
type MouseEnterContextType = [boolean, React.Dispatch<React.SetStateAction<boolean>>];

//  Give it a default so it is never undefined.
...
// We restrict translations/rotations to `number`. That way, we always append "px" or "deg" correctly.
...
export const useMouseEnter = (): MouseEnterContextType => {
  ...
  if (!context) {
    throw new Error("useMouseEnter must be used within a CardContainer provider");
  }
  return context;
};
